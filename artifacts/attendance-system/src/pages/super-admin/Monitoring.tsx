...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span style={{
      ...
      background: ok ? "rgba(16,185,129,0.15)" : "rgba(248,113,113,0.15)",
      color: ok ? "#10b981" : "#f87171",
      border: `1px solid ${ok ? "rgba(16,185,129,0.3)" : "rgba(248,113,113,0.3)"}`,
    }}>
      <span style={{ width: 6, height: 6, borderRadius: "50%", background: ok ? "#10b981" : "#f87171",
        boxShadow: ok ? "0 0 6px #10b981" : "0 0 6px #f87171", animation: ok ? "pulse 2s infinite" : "none" }} />
      {ok ? "متصل" : "غير متصل"}
    </span>
  );
}
...
export default function MonitoringPage() {
  ...
  const historyChart = history.cpu.map((cpu, i) => ({
    idx: i, cpu, ram: history.ram[i] ?? 0,
  }));

  const s = { fontFamily: "'Tajawal', sans-serif", color: "#fff" };

  if (loading) return (
    <div style={{ minHeight: "100vh", background: "#020817", display: "flex", alignItems: "center", justifyContent: "center", ...s }}>
  ...
  return (
    <div dir="rtl" style={{ minHeight: "100vh", background: "linear-gradient(135deg,#020817,#050d1f,#080318)", ...s }}>
      ...
      {/* Header */}
      <div style={{ background: "rgba(2,8,23,0.85)", borderBottom: "1px solid rgba(0,245,255,0.1)", backdropFilter: "blur(20px)", position: "sticky", top: 0, zIndex: 10 }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "14px 20px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div>
              <h1 style={{ fontSize: 16, fontWeight: 800, margin: 0 }}>مركز مراقبة النظام</h1>
              <p style={{ fontSize: 11, color: "rgba(0,245,255,0.5)", margin: 0 }}>
                آخر تحديث: {lastRefresh.toLocaleTimeString("ar")} · يتحدث كل 5 ثوان
              </p>
            </div>
          </div>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "24px 20px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: 14, marginBottom: 24 }}>
          {[
            { icon: <Server size={18} />, label: "حالة الخادم", value: data?.status === "online" ? "متصل" : "غير متصل", color: "#10b981", extra: data?.platform },
            { icon: <Zap size={18} />, label: "الرد المتوسط", value: `${data?.api.avgResponseMs ?? 0} ms`, color: data && data.api.avgResponseMs < 200 ? "#10b981" : data && data.api.avgResponseMs < 500 ? "#f59e0b" : "#f87171", extra: `${data?.api.requestsPerMin ?? 0} req/min` },
            { icon: <Users size={18} />, label: "جلسات نشطة", value: String(data?.activeSessions ?? 0), color: "#00f5ff", extra: `${data?.api.totalRequests ?? 0} طلب إجمالي` },
            { icon: <AlertTriangle size={18} />, label: "أخطاء أخيرة (ساعة)", value: String(data?.recentErrors ?? 0), color: data?.recentErrors ? "#f87171" : "#10b981", extra: data?.nodeVersion },
            { icon: <Clock size={18} />, label: "وقت التشغيل", value: `${data?.uptime.days ?? 0}ي ${data?.uptime.hours ?? 0}س`, color: "#a855f7", extra: `${data?.uptime.minutes ?? 0} دقيقة` },
          ].map((item, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} style={card()}>
              ...
              <div style={{ fontSize: 10, color: "rgba(255,255,255,0.3)", marginTop: 2 }}>{item.extra}</div>
            </motion.div>
          ))}
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 24 }}>
          {[
            { label: "المعالج CPU", value: data?.cpu.usage ?? 0, color: data && data.cpu.usage > 80 ? "#f87171" : data && data.cpu.usage > 60 ? "#f59e0b" : "#00f5ff", extra: `${data?.cpu.cores ?? 0} نواة`, icon: <Cpu size={16} /> },
            { label: "الذاكرة RAM", value: data?.ram.percent ?? 0, color: data && data.ram.percent > 85 ? "#f87171" : data && data.ram.percent > 65 ? "#f59e0b" : "#a855f7", extra: `${data?.ram.usedGb ?? 0} / ${data?.ram.totalGb ?? 0} GB`, icon: <HardDrive size={16} /> },
          ].map((item, i) => (
            <motion.div key={i} style={{ ...card(), display: "flex", alignItems: "center", gap: 24 }}>
              <div style={{ position: "relative", flexShrink: 0 }}>
                <GaugeRing value={item.value} color={item.color} size={96} />
              ...
              <div>
                ...
                <div style={{ fontSize: 12, color: "rgba(255,255,255,0.4)" }}>{item.extra}</div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 24 }}>
          ...
          <div style={card()}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
              ...
              <span style={{ fontWeight: 700, fontSize: 13 }}>سجل الموارد</span>
            </div>
            <ResponsiveContainer width="100%" height={160}>
              <AreaChart data={historyChart}>
                ...
                <Tooltip contentStyle={{ background: "#1a1a2e", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, fontSize: 11 }} formatter={(v: any, n: string) => [`${v}%`, n === "cpu" ? "CPU" : "RAM"]} />
            ...
            <div style={{ display: "flex", gap: 16, marginTop: 8, justifyContent: "center" }}>
              {[{ c: "#00f5ff", l: "CPU" }, { c: "#a855f7", l: "RAM" }].map(x => (
                <span key={x.l} style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 10, color: "rgba(255,255,255,0.4)" }}>
          ...
          <div style={card()}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
              ...
              <span style={{ fontWeight: 700, fontSize: 13 }}>معدل الطلبات (آخر 10 دقائق)</span>
            </div>
            <ResponsiveContainer width="100%" height={160}>
              <BarChart data={data?.chartData ?? []}>
                ...
                <Bar dataKey="requests" fill="#10b981" radius={[4, 4, 0, 0]} opacity={0.8} name="طلبات" />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 24 }}>
          <div style={card()}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
              ...
              <span style={{ fontWeight: 700, fontSize: 13 }}>قاعدة البيانات</span>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
              ...
              <div>
                <div style={{ fontSize: 16, fontWeight: 700, color: data?.database.status === "online" ? "#10b981" : "#f87171" }}>
                  {data?.database.status === "online" ? "PostgreSQL متصل" : "قاعدة البيانات غير متاحة"}
                </div>
                <div style={{ fontSize: 11, color: "rgba(255,255,255,0.35)", marginTop: 2 }}>{data?.database.host}</div>
              </div>
            </div>
          </div>
          <div style={card()}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
              ...
              <span style={{ fontWeight: 700, fontSize: 13 }}>أداء الـ API</span>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, textAlign: "center" }}>
              {[
                { label: "متوسط الرد", value: `${data?.api.avgResponseMs ?? 0}ms`, color: "#00f5ff" },
                { label: "طلب/دقيقة", value: String(data?.api.requestsPerMin ?? 0), color: "#10b981" },
                { label: "إجمالي الطلبات", value: String(data?.api.totalRequests ?? 0), color: "#a855f7" },
              ].map(x => (
                <div key={x.label} style={{ background: "rgba(255,255,255,0.03)", borderRadius: 10, padding: "10px 8px" }}>
                  <div style={{ fontSize: 16, fontWeight: 800, color: x.color }}>{x.value}</div>
                  <div style={{ fontSize: 10, color: "rgba(255,255,255,0.3)", marginTop: 2 }}>{x.label}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <div style={card()}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <span style={{ fontWeight: 700, fontSize: 13 }}>سجل العمليات الأخيرة</span>
            </div>
            <button onClick={fetchAuditLogs} style={{ fontSize: 11, color: "#a855f7", background: "none", border: "none", cursor: "pointer" }}>تحديث</button>
          </div>
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
              <thead>
                <tr style={{ color: "rgba(255,255,255,0.3)", fontSize: 11 }}>
                  {["المستخدم", "العملية", "المورد", "الشركة", "IP", "الوقت", "الحالة"].map(h => (
              ...
              <tbody>
                {auditLogs.map((log, i) => (
                  <tr key={log.id} style={{ borderTop: "1px solid rgba(255,255,255,0.04)", background: i % 2 === 0 ? "rgba(255,255,255,0.01)" : "transparent" }}>
                    <td style={{ padding: "7px 10px", color: "#fff" }}>{log.user_name ?? "—"}</td>
                    ...
                    <td style={{ padding: "7px 10px", color: "rgba(255,255,255,0.5)" }}>{log.resource}</td>
                    <td style={{ padding: "7px 10px", color: "rgba(255,255,255,0.4)" }}>{log.company_name ?? "—"}</td>
                    <td style={{ padding: "7px 10px", color: "rgba(255,255,255,0.35)", fontFamily: "monospace" }}>{log.ip_address ?? "—"}</td>
                    <td style={{ padding: "7px 10px", color: "rgba(255,255,255,0.3)", whiteSpace: "nowrap" }}>
                      {new Date(log.created_at).toLocaleString("ar-IQ", { hour12: false })}
                    </td>
                    <td style={{ padding: "7px 10px" }}>
                ...
                {auditLogs.length === 0 && (
                  <tr><td colSpan={7} style={{ textAlign: "center", padding: 24, color: "rgba(255,255,255,0.2)" }}>لا يوجد سجلات</td></tr>
                )}
              </tbody>
            </table>
          </div>
